...
Per standing preference, only use this loop for **complex tasks (roughly ≥4 tasks, or cross-cutting/multi-file work)** or when he explicitly requests it. If the work is small/sequential and you can do it directly with a quick self-review, say so and just do it — don't spin up the loop for its own sake.
