...
- `all` → audit every repo under `/mnt/d/Documents/Code/GitHub/` (and `Web/`),
  one `audit.py` run each, then present a single table sorted by severity.
  Do **not** fix anything in `all` mode — report only.
